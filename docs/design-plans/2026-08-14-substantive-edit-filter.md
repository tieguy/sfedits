...
- **`recentchanges`**: An Action API list of recent edits on a wiki; used here to sample validation-cohort edits in daily slices across a ~30-day window.
...
| `references` | `doc.references()`, with raw wikitext fallback for empty JSON | substantive | Sorted (unordered set) |
| `media` | `doc.images()` with file, caption, and alt text | substantive | Sorted (unordered set) |
| `tables` | `doc.tables()` with normalized wikitext for captions/headers | substantive | Document order (structural) |
| `headings` | section titles | substantive | Document order (structural) |
| `redirect` | redirect target page and anchor from `doc.redirectTo()` | substantive | N/A (single value per page) |
...
**Pipeline hook — a significance stage in `page-watch.js`, in `lib/significance-stage.js`.** The stage runs inside `sendStatus()` after the metadata filters and **before the diff-HTML fetch** — still post-collapse, once per edit, applied per-consumer. Running before the diff fetch means a dropped edit skips the diff/image work entirely, and the `--noop` path (which returns before the diff fetch) still exercises the stage, so verdicts are observable in a local no-post run. The stage logic lives in `lib/significance-stage.js` with the fetch and classify functions injected, so the conservative-pass behavior is unit-testable; `page-watch.js` stays a thin call. If any surviving consumer sets `substantive_only`, the stage fetches the two revisions, calls `classifyEdit()` once (channel overrides taken only from opted-in consumers), and applies the single verdict to each opted-in consumer. Consumers without the flag are untouched.
...
**Failure policy — conservative pass, at two layers.** The classifier itself returns a fallback verdict (`substantive: true` with `fallback: 'missing-content' | 'input-too-large' | 'parse-error'`) whenever it cannot classify — parse exceptions included — so the module honors the policy for any caller, not just this pipeline. The pipeline stage adds a second layer: fetch failure or any throw inside the stage ⇒ the edit posts, with a logged warning. The filter may only remove noise, never silently eat real edits. This matches the conservative-bias convention already documented in `lib/edit-filters.js`.
...
- Cohort data under `data/edit-significance-validation/` (added to `.gitignore` as part of this phase) — SFBA cohort of 1,219 labeled edits already present; add ~1,500 enwiki mainspace edits and ~1,000 eswiki edits via `list=recentchanges`, sampled in daily slices across a ~30-day window so the cohorts aren't a single time-of-day slice, labeled with mwedittypes (`lang='es'` for eswiki)
...
**Done when:** the directional gate holds per cohort — of edits mwedittypes labels prose-touching, ≥95% classify substantive (denominator: prose-labeled edits; the we-say-substantive-only bucket is characterized, not gated); ~30 disagreements per cohort hand-reviewed as judgment differences, not parser bugs; analysis doc written. If a cohort fails the bar, the confusion cases feed back into Phase 1.
...
**Components:**
- `substantive_only` + channel overrides in `lib/edit-filters.js` (`DEFAULTS`, `normalizeEditFilters()`, a needs-significance predicate, a pure drop-reason helper)
- `lib/revision-pair.js` (batched `revids` + tags fetch) and `lib/significance-stage.js` (stage logic with injected fetch/classify, so conservative-pass is unit-tested); `page-watch.js` wires the stage in after the metadata filters and before the diff fetch
