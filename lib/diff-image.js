...
async function fetchArticleMeta(diffUrl, page) {
  ...
  if (!params) return { description: null, imageDataUri: null, thumbnailUrl: null }
  ...
  return { description, imageDataUri, thumbnailUrl }
}
    ...
    if (diff && diff.length > 0) {
      ...
      if (screenshot) {
        return {
          ...
          article: { description: meta.description, thumbnailUrl: meta.thumbnailUrl }
        }
      }
    }
  } catch (error) {
