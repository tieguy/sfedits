 ...
 * Requests are serialized to avoid parallel fan-out at Wikimedia hosts.
 */
async function fetchArticleMeta(diffUrl, page) {
  ...
  // Serial requests: fetchPageSummary first, then fetchBlpStatus
  const { description, thumbnailUrl } = await fetchPageSummary(params.host, page)
  const blpStatus = await fetchBlpStatus(params.host, page)
 ...
 *   ({description, thumbnailUrl, blp}). Returns null if compare fetch fails,
 *   the diff is empty, or native rendering fails.
 */
async function captureDiffImage(diffUrl, page) {
  try {
    // Serial requests: fetchCompareDiff first, then fetchArticleMeta
    const diff = await fetchCompareDiff(diffUrl)
    const meta = await fetchArticleMeta(diffUrl, page)
