 ...
 *
 * Both long-running processes use this - the `bot` job and the `web`
 * webservice, which a deploy restarts the same way - so every entry point is
 * named and keeps its own record.
 */
...
const EXIT_CODES = {
  0: 'clean exit (handled a stop signal, or finished normally)',
...
/**
 * Where a process's run record lives. $TOOL_DATA_DIR is the tool's NFS home in
 ...
 *
 * The name is the Procfile process ('bot', 'web'). They share the state
 * directory, so each needs its own file - one record per process, not one
 * record the two of them overwrite in turn.
 *
 * @param {string} [name]
 */
function runRecordPath(name = 'bot') {
  const stateDir = process.env.SFEDITS_STATE_DIR ||
    path.join(process.env.TOOL_DATA_DIR || os.homedir(), 'data')
  return path.join(stateDir, `last-run-${name}.json`)
}
...
function readRecord(name) {
  try {
    return JSON.parse(fs.readFileSync(runRecordPath(name), 'utf8'))
  } catch {
 ...
 * @param {string} [name] - Procfile process name ('bot', 'web')
 * @returns {object|null} the previous run's record, if there was one
 */
function startRun(name = 'bot') {
 ...
 * @param {string} [name]
 */
function stopRun(reason, exitCode = 0, name = 'bot') {
  recorded = true
  writeRecord(name, {
    status: 'stopped',
    pid: process.pid,
 ...
 * @param {string} [options.name] - Procfile process name, for the run record
 * @returns {function} removes the handlers again (for tests)
 */
function installStopHandlers({ cleanup, graceMs = DEFAULT_GRACE_MS, name = 'bot' } = {}) {
  let stopping = false
  ...
  const exitHandler = (code) => {
    if (!recorded) stopRun(`exit ${code}`, code, name)
  }
