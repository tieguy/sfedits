...
async function deliver(subscription, payload, options = {}) {
  ...
  try {
    ...
    if (result === null) {
      // Webhook was already validated above, but delivery.post can also
      // validate and reject (e.g. if allowlist changes). Return permanent failure.
      return {
        ok: false,
        capped: false,
        subscriptionId: subscription.id,
        error: 'webhook URL rejected by allowlist',
        permanent: true
      }
    }
