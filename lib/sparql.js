...
async function sparqlRaw(query) {
  ...
  try {
    res = await wmFetch(SPARQL_URL, {
      ...
      // WDQS 429s: honor Retry-After in-band, but briefly — after 3 waits
      // wmFetch throws and the outer isRetryable/onChunkError path takes over,
      // as it does today. Without this cap, defaults would block one sparqlRaw
      // call for up to 10 minutes.
      maxRateLimitWaits: 3
    })
  } catch (error) {
    // wmFetch's capped-429 throw carries no .status; restore it so
    // isRetryable's 429 branch keeps classifying it as retryable.
    if (error.message === 'HTTP 429') error.status = 429
    throw error
  }

  if (!res.ok) {
    const body = await res.text().catch(() => '')
    const error = new Error(`SPARQL returned ${res.status}`)
