...
async function sparqlSelect(query) {
  ...
  if (!response.ok) {
    throw new Error(`SPARQL returned ${response.status}`)
  }
  const data = await response.json()
  return data.results.bindings.map(b =>
    Object.values(b)[0].value.replace('http://www.wikidata.org/entity/', ''))
}
...
async function handleWikidataEdit(account, edit, { sets, noop = false, rateCap = null }) {
  ...
  const verb = parsed.action === 'changed' ? 'changed to' : parsed.action
  const embed = {
    title: `Wikidata: ${labels[edit.page]}`,
    url: edit.url,
    description:
      `**${labels[parsed.property]}**: ${labels[parsed.value]} ${verb}` +
