 ...
 *
 * Dedupe: the Wikidata UI turns one human action into several edits - adding
 * a statement and then attaching its reference arrives as wbsetclaim-create
 * followed seconds later by wbsetclaim-update, both carrying the same
 * property and value. A short-TTL memory of recently posted
 * (page, property, value) keys collapses that burst into one notice.
 */
...
const USER_AGENT = userAgent('claim-watch')
...
const DEFAULT_DEDUPE_SECONDS = 45
const CACHE_FILE = 'wikidata-claim-targets.json'

/**
 * Short-TTL memory of recently posted claims, keyed by
 * (page, property, value). Collapses the create-then-refine bursts the
 * Wikidata UI produces into a single notice.
 *
 * A repeat within the TTL is suppressed when it is a 'changed' action
 * (create-then-touch, the common burst) or repeats the previous action
 * (stream replays after a reconnect). An action that flips the claim's
 * presence - removed-after-added, re-added-after-removed - always posts:
 * suppressing those would leave the channel showing stale state.
 */
class ClaimDedupe {
  constructor({ ttlMs = DEFAULT_DEDUPE_SECONDS * 1000, now = Date.now } = {}) {
    this.ttlMs = ttlMs
    this.now = now
  ...
  /** True when a notice for this claim should be suppressed as a duplicate */
  isDuplicate({ page, property, value, action }) {
    this._prune()
    const entry = this.entries.get(`${page}|${property}|${value}`)
    if (!entry) return false
    return action === 'changed' || action === entry.action
  }

  /** Record a posted notice so near-future repeats can be suppressed */
  record({ page, property, value, action }) {
    this._prune()
    this.entries.set(`${page}|${property}|${value}`, { action, at: this.now() })
  }
}
...
/**
 * Fetch the item-valued claims a property held in a given (old) revision, so
 * a 'changed' notice can say what the value changed *from* - the autocomment
 * only carries the new value. Returns an array of Q-ids, or null when the
 * old revision can't be read (fetch failure, deleted revision, ...).
 */
async function fetchPreviousClaimValues(oldRevId, property) {
  try {
    const params = new URLSearchParams({
      action: 'query',
      revids: String(oldRevId),
      prop: 'revisions',
      rvprop: 'content',
      rvslots: 'main',
      format: 'json',
    ...
    if (!response.ok) throw new Error(`query revisions returned ${response.status}`)
    const data = await response.json()
    ...
    console.error(`[claim-watch] could not read old revision ${oldRevId}:`, error.message)
    return null
  }
}

/** Pull the oldid query param from a wikichanges-style diff URL */
function oldRevIdFromUrl(editUrl) {
  try {
    return new URL(editUrl).searchParams.get('oldid')
  } catch {
    return null
  }
}
 ...
 * @param {ClaimDedupe} [deps.dedupe] - Shared per-account duplicate memory
 * @returns {Promise<Object|null>}
 */
async function handleWikidataEdit(account, edit, { sets, noop = false, rateCap = null, dedupe = null }) {
  ...
  const claimKey = { page: edit.page, property: parsed.property, value: parsed.value, action: parsed.action }
  if (dedupe && dedupe.isDuplicate(claimKey)) {
    console.log(`[claim-watch] deduped: ${edit.page} ${parsed.property} ${parsed.action}`)
    return null
  }
  ...
  // A wbsetclaim-update autocomment fires for any touch of the claim -
  // reference and qualifier edits included - and always carries the claim's
  // current value, so "changed" alone can't be trusted to mean the value
  // changed. Read the parent revision to say what actually happened.
  let priorValues = null
  if (parsed.action === 'changed') {
    const oldRevId = oldRevIdFromUrl(edit.url)
    if (oldRevId) {
      priorValues = await fetchPreviousClaimValues(oldRevId, parsed.property)
    }
  }

  const valueUnchanged = priorValues !== null && priorValues.includes(parsed.value)
  // The autocomment doesn't say which statement was edited, so a single
  // prior value is the only case where "changed from X" is unambiguous
  const priorValue = (priorValues !== null && !valueUnchanged && priorValues.length === 1)
    ? priorValues[0]
    : null

  const labelIds = [edit.page, parsed.value, parsed.property]
  if (priorValue) labelIds.push(priorValue)
  const labels = await fetchLabels(labelIds)

  const action = ACTION_LABELS[parsed.action] || `**${parsed.action}**`
  let detail
  if (valueUnchanged) {
    detail = `✏️ **Edited** — **${labels[parsed.property]}**: ${labels[parsed.value]} (reference or qualifier change)`
  } else if (priorValue) {
    detail = `✏️ **Changed** — **${labels[parsed.property]}**: ${labels[priorValue]} → ${labels[parsed.value]}`
  } else {
    detail = `${action} — **${labels[parsed.property]}**: ${labels[parsed.value]}`
  }
  ...
  if (dedupe) dedupe.record(claimKey)
...
async function startClaimWatch(config, { dataDir }) {
  ...
  for (const account of config.accounts || []) {
    ...
    account.claimWatch = {
      ...
      dedupe: new ClaimDedupe({
        ttlMs: (claimsConfig.dedupe_seconds ?? DEFAULT_DEDUPE_SECONDS) * 1000
      })
    }
