...
async function postBluesky(delivery, payload) {
  // Bluesky's 300-grapheme fitting happens inside blueskyPlatform.post
  // (lib/bluesky-platform.js), which shortens the title and keeps facets in
  // sync itself — delivery passes text and metadata through untouched.
...
function _resolveDeliveryEntry(account, entry) {
  ...
  // A non-default credential_ref is rejected until the posted log records the
  // credential source: the revdel sweeper resolves deletes from the account's
  // DEFAULT stanza (lib/revdel-check.js), so a post made with alternate
  // credentials could never be taken down — Discord would 404 and be marked
  // "already gone" while the post stayed up. Fail at config load, not at sweep.
  if (credential_ref !== type) {
    throw new Error(
      `Delivery entry for type "${type}" uses credential_ref "${credential_ref}": ` +
      'non-default credential_ref is not supported until the revdel sweeper can resolve it'
    )
  }
