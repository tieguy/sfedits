 ...
 * @returns {Promise<{type, postId, ref} | null>} {type, postId, ref} on success, null on validation failure
 * @throws {Error} For unknown platform types; platform errors propagate untouched
 */
...
async function postBluesky(delivery, payload) {
  ...
  // Guard against platforms yielding no uri
  if (!result?.uri) {
    return null
  }
...
async function postDiscord(delivery, payload) {
  ...
  // Guard against platforms yielding no id
  if (!result?.id) {
    return null
  }
