...
function normalizeEditFilters(filters) {
  ...
  return {
    ...
    substantive_channels: normalizeSubstantiveChannels(filters.substantive_channels)
  }
}

// Only 'substantive' and 'ignored' are valid channel policies. Anything else
// (a typo like "subtantive") is dropped so the channel keeps its default —
// a bad override must never silently weaken the filter into dropping edits.
function normalizeSubstantiveChannels(channels) {
  if (!channels || typeof channels !== 'object' || Array.isArray(channels)) return null
  const valid = {}
  for (const [name, policy] of Object.entries(channels)) {
    if (policy === 'substantive' || policy === 'ignored') valid[name] = policy
  }
