...
function extractChannels(doc) {
  ...
  // References: include wikitext when JSON lacks data (handles Spanish citation templates)
  const refExtractor = doc.references().map(r => {
    const json = r.json()
    // If json has no meaningful data (common with non-English citation templates),
    // fall back to canonicalized wikitext to catch title/content changes
    if (json.data && Object.keys(json.data).length === 0 &&
        json.inline && Object.keys(json.inline).length === 0) {
      return { wikitext: canonicalizeWikitext(r.wikitext()) }
    }
    return json
  })
