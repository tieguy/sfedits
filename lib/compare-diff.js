...
async function fetchBlpStatus(host, page) {
  try {
    ...
    const qid = ppData?.query?.pages?.[0]?.pageprops?.wikibase_item
    if (!qid) return { isBlp: null, reason: null, qid: null }
    ...
    // away from fixable
    return { ...blpFromClaims(instanceOfIds, deathTime), qid }
  } catch (e) {
    return { isBlp: null, reason: null, qid: null }
  }
}
