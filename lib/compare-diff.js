 ...
 * markup classification). `addedLines`/`removedLines` regroup them by source
 * line, joining a line's several highlight ranges with an ellipsis, so
 * display surfaces (Discord embeds) put one diff line on one line instead of
 * breaking mid-sentence at every highlight boundary.
 */
  ...
  for (const line of diff) {
    if (line.type === TYPE_ADD) {
      ...
      for (const seg of stripSegments(splitHighlights(line.text, line.highlightRanges))) {
        ...
        if (!t) continue
        if (seg.highlight === 'add') lineAdded.push(t)
        else if (seg.highlight === 'delete') lineRemoved.push(t)
      }
      added.push(...lineAdded)
      removed.push(...lineRemoved)
      if (lineAdded.length) addedLines.push(lineAdded.join(' … '))
      if (lineRemoved.length) removedLines.push(lineRemoved.join(' … '))
    }
  }
...
function renderSegments(text, highlightRanges) {
  return displaySegments(text, highlightRanges).map(seg => {
 ...
 * styled text segments. Consumed by renderDiffHtml (browser screenshot
 * path) and lib/diff-render-native.js (satori/resvg, no browser).
 *
...
function buildDiffModel(diff, page, meta = {}) {
  ...
  const plainRow = (kind, gutter, text, max) => ({
    kind,
    gutter,
    segments: [{ text: truncate(stripWikitext(text), max), highlight: null }]
  })
...
function renderDiffHtml(diff, page, meta = {}) {
  ...
  const rows = model.rows.map(row => {
    const content = row.segments.map(seg => {
      const escaped = escapeHtml(seg.text)
      if (seg.highlight === 'add') return `<ins>${escaped}</ins>`
      if (seg.highlight === 'delete') return `<del>${escaped}</del>`
      return escaped
    }).join('')
    return `<div class="row ${row.kind}"><span class="gutter">${row.gutter}</span><span class="text">${content || '&nbsp;'}</span></div>`
  })
