...
const { buildFacets, fitBlueskyText } = require('./bluesky-utils')
...
async function post({ account, text, screenshot, metadata, replyTo }) {
  ...
  const fitted = fitBlueskyText(text, metadata.page)
  ...
  const record = {
    ...
    embed: {
      ...
      images: [{
        // Alt text is prose, not post text — no grapheme limit applies, so the
        // original (un-fitted) title reads better than the ellipsized one.
