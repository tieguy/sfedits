...
async function actionSession(host, component, { timeoutMs = ACTION_SESSION_HEADERS_TIMEOUT_MS, requestDeadlineMs = ACTION_SESSION_REQUEST_DEADLINE_MS } = {}) {
  ...
  const promise = loadM3api().then(({ default: Session }) => {
    ...
    // Wrap session.request with deadline enforcement to catch wedged dispatchers.
...
// Wrap a session's request method with out-of-band deadline enforcement.
...
// Uses Promise.race with AbortSignal.timeout to enforce the deadline.
...
function wrapSessionWithDeadline(session, deadlineMs) {
  ...
  session.request = async (params, options) => {
    ...
    try {
      return await originalRequest(params, { ...options, signal: composedSignal })
    } catch (error) {
      // If it's a timeout from our deadline, provide a clear message
      if (error.name === 'AbortError' && deadlineSignal.aborted && !options?.signal?.aborted) {
