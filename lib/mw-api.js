...
// Generous out-of-band deadline for the entire session request lifecycle,
// including all retries and Retry-After waits. This is a last-resort guard
// that catches wedged dispatchers or unexpected hangs, sitting well above
// m3api's retry budgets (which max out at ~600s = 10min). The default is
// 15 minutes — comfortably above retry logic but ensures no hang lasts forever.
// If a dispatcher is broken for a Node version, this deadline will reject
...
// a generous out-of-band timeout for the entire request lifecycle (default 15min),
// sitting above all retry budgets to catch wedged dispatchers.
...
// Wrap a session's fetch method with out-of-band deadline enforcement.
// The deadline is a last-resort guard sitting above m3api's retry budgets,
// catching wedged dispatchers that otherwise wouldn't timeout.
// Overrides session.fetch to inject the deadline signal into all fetch calls
// (used by both session.request and m3api-rest). Each fetch attempt gets its
// own fresh deadline signal, .unref'd to not hold the event loop.
...
function wrapSessionWithDeadline(session, deadlineMs) {
  ...
  session.fetch = async (url, options = {}) => {
    ...
    // This is a last-resort guard and sits above retry budgets.
