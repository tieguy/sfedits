...
// (gzip/deflate/br by default) — invisible to nock, verified live in Phase 5.
async function actionSession(host, component, { timeoutMs = ACTION_SESSION_HEADERS_TIMEOUT_MS } = {}) {
  ...
  const promise = loadM3api().then(({ default: Session }) => {
    ...
    return createTimeoutAgent(session, timeoutMs)
  }).catch(error => {
...
async function createTimeoutAgent(session, headersTimeoutMs = ACTION_SESSION_HEADERS_TIMEOUT_MS) {
  ...
  session.agent = new CookieAgent({
    ...
    bodyTimeout: ACTION_SESSION_BODY_TIMEOUT_MS
  })
  return session
}
