...
async function actionSession(host, component, { timeoutMs = ACTION_SESSION_HEADERS_TIMEOUT_MS, requestDeadlineMs = ACTION_SESSION_REQUEST_DEADLINE_MS, bare429WaitMs = BARE_429_WAIT_MS } = {}) {
  // component is required for attribution; match user-agent.js's contract
  if (!component || typeof component !== 'string') {
      ...
      httpErrorHandlers: [
        ...
        async (session, params, options, response) => {
          if (response.status === 429 && !response.headers.has('retry-after')) {
            ...
            const retryBudgetMs = options.retryUntil - performance.now()
            if (retryBudgetMs < bare429WaitMs) return null

            console.warn(`[actionSession] bare 429: waiting ${bare429WaitMs}ms before retry`)
            await sleep(bare429WaitMs)
