...
/**
 * Wikidata properties that say the SUBJECT IS in the region, as opposed to
 * merely having passed through it: located in it (P131*), headquartered
 * (P159) or located (P276) there, or holding an office whose jurisdiction is
 * there (P39). Born/died/worked-here (P19/P20/P937) are deliberately absent -
 * measured on the Bay Area cohort they contributed ~10,900 of ~15,500 raw
 * candidates and almost none were plausible members. Reagan worked at Fort
 * Mason and Microsoft has a Silicon Valley campus; both true, neither a Bay
 * Area topic. scripts/reassess.js ranks over this same set - the universe
 * rule and the membership rule are one decision, defined here.
 */
const IS_HERE_PROPERTIES = ['P131', 'P159', 'P276', 'P39']

/** Regex for a valid Wikidata property id: P followed by digits, no leading zeros. */
const PROP = /^P[1-9]\d*$/

/**
 * Validate a property id string and throw a descriptive error if invalid.
 * Property ids are interpolated into SPARQL, so this is an injection boundary.
 */
function assertProp(prop) {
  if (typeof prop !== 'string' || !PROP.test(prop)) {
    throw new Error(`Wikidata property must look like "P131", got ${JSON.stringify(prop)}`)
  }
  return prop
}

/**
 * SPARQL graph pattern binding ?item to the members of a region via one
 * property. Three shapes, because the properties relate differently:
 *
 *  - P131: the item itself is located in the region (transitive closure).
 *  - P39: the item holds an office (?pos) whose jurisdiction (P1001) is the
 *    region or anything located in it. The union is anchored at the region on
 *    both branches - the unanchored form times out on WDQS (learned in the
 *    claim-watch work; lib/wikidata-claim-watch.js runs the same shape).
 *  - anything else (P159, P276, and reassess-untagged's wider P19/P20/P937):
 *    the item points at a place (?place) located in the region.
 *
 * Callers compose their own SELECT around this: lib/region.js projects
 * sitelinks per language, scripts/reassess-untagged.js projects enwiki titles.
 * The pattern reserves the variable names ?place, ?pos and ?j.
 */
function memberPattern(prop, anchorQid) {
  assertProp(prop)
  assertQid(anchorQid, 'Anchor QID')

  if (prop === 'P131') {
    return `?item wdt:P131* wd:${anchorQid} .`
  }
  if (prop === 'P39') {
    return `?item wdt:P39 ?pos .
      { ?pos wdt:P1001 wd:${anchorQid} }
      UNION { ?j wdt:P131+ wd:${anchorQid} . ?pos wdt:P1001 ?j }`
  }
  return `?item wdt:${prop} ?place . ?place wdt:P131* wd:${anchorQid} .`
}
...
/**
 * Administrative membership: everything Wikidata says IS in the region -
 * located in it, headquartered or located there, or holding an office with
 * jurisdiction there - with its Wikipedia articles in the requested languages.
 *
 * One serial query per property rather than one UNION: Wikimedia etiquette
 * forbids fan-out, and anchored single-property queries are the shape WDQS
 * resolves without timeouts where multi-branch unions do not.
 *
 * @param {Object} region - descriptor from resolveRegion()
 * @param {Object} [options]
 * @param {string[]} [options.languages] - wiki language codes; omit for all
 * @param {string[]} [options.properties] - membership properties; defaults to
 *   IS_HERE_PROPERTIES
 * @returns {Promise<Array<{qid, cls, lang, wikipedia, title, source}>>}
 */
async function articlesByAdmin(region, options = {}) {
  // region.qid is interpolated straight into the queries below, so it is the
  // injection boundary and must be validated here (memberPattern re-checks).
  assertQid(region.qid, 'Region QID')

  const { languages = null, properties = IS_HERE_PROPERTIES } = options
  ...
  for (const prop of properties) {
    ...
    for (const row of rows) {
      ...
      articles.push({
        ...
        // 'admin' predates the wider membership and is what stored topics
        // carry for located-in rows; the other properties name themselves.
        source: prop === 'P131' ? 'admin' : prop
      })
    }
  }
...
async function regionHistogram(region, options = {}) {
  ...
  // region.qid is interpolated into the queries, so it is the injection boundary.
  assertQid(region.qid, 'Region QID')

  const { properties = IS_HERE_PROPERTIES } = options

  // One aggregation per membership property, cells merged across them. An item
  // matched by two properties counts twice - the histogram is an estimate by
  // design and the dry run remains the source of truth, but the estimate has
  // to WIDEN with membership or the create-form's max_articles gate would
  // undercount office holders and headquartered orgs.
  const merged = new Map()
  for (const prop of properties) {
    const rows = await runRegionQuery(region, `
    ...
    // Within one response a GROUP BY returns each (cls, lang) once, but merge
    // defensively so a duplicated row cannot overwrite or double-list a cell.
    for (const row of rows) {
  ...
  // Each query either succeeds whole or throws; there is no partial state.
