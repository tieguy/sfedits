...
async function subEntities(regionQid) {
  ...
  `)
  // Filter to only valid QIDs. Each sub is interpolated into `wd:${anchor}` below, so
  // this filter is the boundary guaranteeing only clean QIDs reach a query string.
  const subs = rows.map(r => r.sub).filter(s => QID.test(s))

  // "No rows at all" and "rows arrived but every one was filtered" mean different
  // things to the caller: the first is a genuinely flat region, the second means we
  // are about to fall back to one unchunked whole-region query - precisely the
  // timeout this module's chunking exists to prevent. Report the difference.
  return { subs, filtered: rows.length - subs.length }
}
