...
/** Regex for a valid language code: 2-3 letter code, optionally with region or script subtags. */
...
function assertLang(lang) {
  if (typeof lang !== 'string' || !LANG.test(lang)) {
    throw new Error(`Language code must be valid BCP 47 format like "en", got ${JSON.stringify(lang)}`)
  }
  return lang
}
...
function parsePoint(wkt) {
  ...
  // Ensure both coordinates are finite numbers, not NaN or Infinity
...
async function resolveRegion(qid, options = {}) {
  ...
  // Each class QID is validated by Wikidata, but we assert it anyway to catch regressions.
  const classes = [...new Set(rows.map(r => r.cls).filter(Boolean).map(c => assertQid(c, 'Class QID')))]
...
async function subEntities(regionQid) {
  ...
  `)
  // Validate all returned QIDs to catch WDQS regressions
  return rows.map(r => assertQid(r.sub, 'Sub-entity QID')).filter(Boolean)
}
