...
const USER_AGENT = userAgent('revdel-check')
...
async function deletePosts(entry, account) {
  ...
  if (updated.mastodonId && !updated.mastodonDeleted) {
    try {
      ...
      await M.deleteStatus(updated.mastodonId)
  ...
  if (updated.discordMessageId && !updated.discordDeleted) {
    if (account.discord?.webhook_url) {
      try {
        const res = await fetch(`${account.discord.webhook_url}/messages/${updated.discordMessageId}`, {
          method: 'DELETE',
          signal: AbortSignal.timeout(FETCH_TIMEOUT_MS)
        })
        ...
        updated.discordDeleted = true
        console.log(`[revdel] Deleted Discord message for ${entry.host}:${entry.revId} (${entry.reason})`)
      } catch (error) {
        console.error(`[revdel] Discord delete failed for ${entry.host}:${entry.revId}: ${error.message}`)
      }
    } else {
      // Webhook no longer configured: nothing we can ever do - don't
      // block the entry from completing
      console.error(`[revdel] No Discord webhook configured - cannot delete message for ${entry.host}:${entry.revId}`)
      updated.discordDeleted = true
    }
  }
  ...
  const discordDone = !updated.discordMessageId || updated.discordDeleted
  if (blueskyDone && mastodonDone && discordDone) {
