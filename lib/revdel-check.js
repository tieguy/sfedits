...
async function deletePosts(entry, account, topicStore) {
  ...
  // Update deliveries array in place if it's the new format
  ...
  // Entry is fully deleted only when every delivery is deleted
  const allDeleted = deliveries.length > 0 && deliveries.every(d => d.deleted)
  if (allDeleted) {
