...
async function refreshWatchlist(account, { dataDir }) {
  ...
  const usesTitleList = Boolean(source.titles_url || source.titles_file)
  const key = sourceKey(source)
  const cacheFile = cachePath(dataDir, key)

  let titles
  try {
    titles = usesTitleList
      ? await fetchTitlesList(source)
      : await fetchProjectArticles(source)
