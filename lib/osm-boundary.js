  ...
  while (unused.size > 0) {
    ...
    while (!ringClosed && unused.size > 0) {
      ...
      for (const wayIdx of unused) {
        ...
        // Way's END meets the ring's start: the way already flows forward
        // into the ring, so prepend it UNREVERSED (drop its last point, the
        // shared node). These two prepend branches had their orientations
        // swapped until 2026-08-03 (LUI-102): the shared node was duplicated
        // at ring[0] and the way's far end buried mid-ring, so multi-way
        // relations like Yosemite consumed every way into one chain that
        // could never close.
        if (ringStart[0] === wayEnd[0] && ringStart[1] === wayEnd[1]) {
          ring = [...way.slice(0, -1), ...ring] // Prepend way forward
          unused.delete(wayIdx)
        ...
        // Way's START meets the ring's start: reverse it so it flows into
        // the ring, then prepend (drop the shared node).
