 ...
 *  @throws {Error} if a ring cannot be closed through stitching
 */
function stitchRings(ways) {
  ...
  while (unused.size > 0) {
    ...
    // Track if this ring consists of a single way (for closeRing fallback)
    const isSingleWay = ringIndices.length === 1
    ...
    while (!ringClosed && unused.size > 0) {
      ...
      if (isClosed(ring)) {
        ...
        if (!isSingleWay) {
          // Multi-way chain failed to close: unrecoverable error
          throw new Error(`OSM relation has an unclosed outer ring (ways stitched: ${ringIndices.join(',')})`)
        }
        // Single way not closed: break and try closeRing fallback below
        break
      }
    }
    ...
    if (ringClosed && ring.length >= 4) {
      ...
      rings.push(ring)
    } else if (!ringClosed && isSingleWay && ring.length >= 3) {
