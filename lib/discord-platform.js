...
const ATTACHMENT_DESCRIPTION_MAX = 1024
 ...
 * bare text). The embed therefore skips those and carries links (title,
 * editor, view diff, action row), BLP status, the change-count summary,
 * and the changed text as copyable Added/Removed fields - text is easier
 * to read on mobile than the image, and the only form a reader can quote
 * or search.
 *
...
function buildDiscordEmbed(metadata, filename) {
  ...
  const fields = []
  // Prefer the per-line grouping: several highlight ranges on one changed
  // line belong on one quoted line, not one apiece.
  const addedLines = summary.addedLines?.length ? summary.addedLines : summary.added
  const removedLines = summary.removedLines?.length ? summary.removedLines : summary.removed
  if (addedLines?.length) {
    fields.push({ name: 'Added', value: formatFragments(addedLines), inline: false })
  }
  if (removedLines?.length) {
    fields.push({ name: 'Removed', value: formatFragments(removedLines), inline: false })
  }
 ...
 * copyable added/removed excerpts, and the diff screenshot (whose header
 * carries the Wikidata description and lead image). Without it (fallback
 * screenshot path), posts the legacy markdown text.
...
async function post({ account, text, screenshot, metadata }) {
  ...
  const payload = {
    ...
    attachments: [{
      id: 0,
      filename,
      // The alt-text generator's own cap (1200) suits Mastodon and Bluesky;
      // Discord rejects attachment descriptions over 1024 with a 400 on
      // attachments[0], so this boundary owns its own clip.
      description: clip(
